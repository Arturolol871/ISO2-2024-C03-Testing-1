...
public class Main {
	...
	public static String generarMensajeMatriculacion(int edad, boolean esEuropea, boolean matricularDoctorado) {
		// Validar si la edad es correcta (no puede ser negativa ni mayor que 120)
		if (edad < 0 || edad > 120) {
			return "Caso no válido: Edad incorrecta";
		}
	
		// Verificar si la persona puede matricularse en el doctorado
		if (edad >= 18 && esEuropea && matricularDoctorado) {
			return "Puede matricularse en el doctorado";
		} else if (edad >= 18 && esEuropea) {
			return "No puede matricularse en el doctorado";
		} else {
			return "No puede matricularse en el doctorado";
		}
	}

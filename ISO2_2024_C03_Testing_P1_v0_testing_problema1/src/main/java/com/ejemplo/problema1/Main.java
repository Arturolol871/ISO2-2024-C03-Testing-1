...
public class Main {
	public static void main(String[] args) {
		 ...
		 boolean certificacionIngles = true;  // Asumiendo que tiene certificaciÃ³n
	...
	public static String leerNombre(Scanner sc) {
		...
		do {
			...
			if (nombre.isEmpty()) {
				System.out.println("El nombre no puede estar vacï¿½o.");
			} else if (nombre.matches(".*\\d.*")) {
				System.out.println("El nombre no puede contener nï¿½meros.");
			} else {
				break;
			}
	...
	public static String leerApellidos(Scanner sc) {
		...
		do {
			...
			if (apellidos.isEmpty()) {
				System.out.println("Los apellidos no pueden estar vacï¿½os.");
			} else if (apellidos.matches(".*\\d.*")) {
				System.out.println("Los apellidos no pueden contener nï¿½meros.");
			} else {
				break;
			}
		...
		do {
			...
			if (!fechaNacimiento.matches("\\d{2}-\\d{2}-\\d{4}")) {
				...
				if (dia < 1 || dia > 31 || mes < 1 || mes > 12 || anio < 1900 || anio > 2024) {
					System.out.println("La fecha introducida no es vï¿½lida.");
				} else {
	...
	public static String leerNacionalidad(Scanner sc) {
		...
		do {
			...
			if (nacionalidad.isEmpty()) {
				System.out.println("La nacionalidad no puede estar vacï¿½a.");
			} else {
				break;
			}
	...
	public static String leerTitulacion(Scanner sc) {
		...
		do {
			System.out.print(
					"Introduce la titulaciï¿½n (Grado, Master, Doctorado): ");
			...
			if (!esValida) {
				System.out.println(
						"La titulaciï¿½n introducida no es vï¿½lida. Debe ser una de las siguientes: Primaria, Secundaria, Bachillerato, Grado, Master, Doctorado.");
			}
	...
	public static boolean leerCertificacionIngles(Scanner sc) {
	    ...
	    while (true) {
	        System.out.print("¿Tiene certificación de inglés? (si/no): ");
	        ...
	        switch (respuesta) {
	            ...
	            default:
	                System.out.println("Por favor, introduce una respuesta válida (si/no).");
	        }
	    }
	}
	...
	public static String leerTelefono(Scanner sc) {
		String telefono;
		do {
			System.out.print("Introduce el nï¿½mero de telï¿½fono (9 dï¿½gitos): ");
			telefono =  sc.nextLine().trim();

			// Verificar que el telï¿½fono tenga 9 dï¿½gitos y que contenga solo nï¿½meros
			if (telefono.length() != 9) {
				System.out.println("El nï¿½mero de telï¿½fono debe tener 9 dï¿½gitos.");
			} else if (!telefono.matches("\\d{9}")) {
				System.out.println("El nï¿½mero de telï¿½fono debe contener solo nï¿½meros.");
			} else {
				break;
			}
		} while (true);
	...
	public static String leerCorreoElectronico(Scanner sc) {
		...
		do {
			System.out.print("Introduce el correo electrï¿½nico: ");
			correoElectronico =  sc.nextLine().trim();

			// Verificar que el correo tenga un formato vï¿½lido
			if (!correoElectronico.matches("^[^@]+@[^@]+\\.[a-zA-Z]{2,}$")) {
				System.out.println("El correo electrï¿½nico no es vï¿½lido.");
			} else {
				break;
			}
		} while (true);
